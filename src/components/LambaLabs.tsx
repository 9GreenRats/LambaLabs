...
import CollectionSetupLamba from './CollectionSetup';
import LayerUploadLamba from './LayerUpload';
import RarityConfigurationLamba from './RarityConfiguration';
import ImageGenerationLamba from './ImageGeneration';
import CollectionAnalyticsLamba from './CollectionAnalytics';
...
const LambaLabs: React.FC = () => {
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
      case 'welcome':
        return (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <h1 className="text-4xl font-bold mb-6 text-indigo-700">Welcome to Lamba Labs NFT Generator</h1>
            <p className="text-xl mb-8 max-w-2xl">Create unique NFT collections with ease using our powerful generative art tool.</p>
      ...
      case 'layers':
        return (
          <DragDropContext onDragEnd={handleOnDragEnd}>
            <LayerUploadLamba layers={layers} setLayers={setLayers} />
          </DragDropContext>
        );
      case 'rarity':
        return <RarityConfigurationLamba layers={layers} setLayers={setLayers} />;
      case 'generation':
        return (
          <ImageGenerationLamba
            layers={layers}
            collectionName={collectionName}
            collectionDescription={collectionDescription}
      ...
      case 'analytics':
        return <CollectionAnalyticsLamba layers={layers} generatedImages={generatedImages} />;
      default:
        return null;
    }
  };
          ...
          <AnimatePresence>
            {!sidebarCollapsed && (
              ...
              >
                ...
                <p className="text-xs text-indigo-300">Generative Art Tool</p>
              </motion.div>
            )}
          </AnimatePresence>
          <button 
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <header className="bg-white shadow-sm p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-indigo-700">{sections.find(s => s.id === activeSection)?.name}</h2>
            {renderBreadcrumbs()}
          </div>
        </header>
